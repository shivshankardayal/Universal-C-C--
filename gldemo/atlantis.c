...
InitFishs(void)
{
    ...
    for (i = 0; i < NUM_DOLPHINS; i++) {
        dolphins[i].x = 40000.0 + rand() % 6000;
        dolphins[i].y = rand() % 6000;
        dolphins[i].z = rand() % 6000;
        dolphins[i].psi = rand() % 360 - 180.0;
        dolphins[i].v = 1.0;
        dolphins[i].theta = 0.0;
    }
...
Animate(void)
{
    ...
    for (i = 0; i < NUM_DOLPHINS; i++) {
		if(rand() % 101) 
        	DolphinPilot(&dolphins[i]);
		else{
        	SharkPilot(&dolphins[i]);
        	DolphinPilot(&dolphins[i]);
	        DolphinMiss(i);
	    }
    }
