...
DolphinMiss(int i)
{
    ...
    for (j = 0; j < NUM_DOLPHINS; j++) {
        if (j != i) {
            X = dolphins[j].x - dolphins[i].x;
            Y = dolphins[j].y - dolphins[i].y;
            Z = dolphins[j].z - dolphins[i].z;

            R = sqrt(X * X + Y * Y + Z * Z);

            avoid = 1.0;
            thetal = dolphins[i].theta;

            if (R < SHARKSIZE) {
                if (Z > 0.0) {
                    dolphins[i].theta -= avoid;
                } else {
                    dolphins[i].theta += avoid;
                }
            }
            dolphins[i].dtheta += (dolphins[i].theta - thetal);
        }
    }
}
