 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: l_video_svgalib.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $
 *
...
#ifndef lint
static const char 
rcsid[] = "$Id: l_video_svgalib.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $";
#endif /* lint */
...
static void I_FindClosestResolution(unsigned width, unsigned height)
{
  ...
  for ( loop = 1; loop <= vlmn; loop++ ) {
    ...
    if ( closest_area == 0 ||
	 abs(closest_area - ideal_area) > abs(area - ideal_area) ||
	 (abs(closest_area - ideal_area) == abs(area - ideal_area) &&
	  abs(SCREENWIDTH - width) > abs(mi -> width - width)) ) {
      closest_area = area;
