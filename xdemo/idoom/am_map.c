 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: am_map.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $
 *
...
mline_t triangle_guy[] =
{
  ...
  { { (fixed_t).867*R, (fixed_t)-.5*R } , { 0, (fixed_t)R } },
  { { 0,(fixed_t) R }, {(fixed_t) -.867*R, (fixed_t)-.5*R } }
};
#undef R
...
{
  { { (fixed_t)-.5*R, (fixed_t)-.7*R }, { (fixed_t)R, 0 } },
  { { (fixed_t)R, 0 }, { (fixed_t)-.5*R, (fixed_t).7*R } },
...
//
// Revision 1.3  2000/08/19 15:46:24  bob
// Fresh update to old source
//
// Revision 1.1  2000/08/07 18:16:10  bob
// Doom port
//
