 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: r_main.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $
 *
...
static const char rcsid[] = "$Id: r_main.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $";
...
#define KEEPTIMES 10
  ...
  if (now - showtime > 35) {
	...
	if(cnt==5){
		cnt=0;
    printf("Frame rate %d fps Segs %d, Visplanes %d, Sprites %d\n", 
		(35*KEEPTIMES)/(now - keeptime[0]), rendered_segs, 
		rendered_visplanes, rendered_vissprites);
	}
	++cnt;
...
//
// Revision 1.8  2000/08/19 15:46:25  bob
