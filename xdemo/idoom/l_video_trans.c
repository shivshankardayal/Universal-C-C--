 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: l_video_trans.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $
 *
...
#ifndef lint
static const char rcsid[] = "$Id: l_video_trans.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $";
#endif /* lint */

#ifdef INTERNETC
#include <stddef.h>
#include <X11/sys/icvm.h>
#endif
...
static void I_Double8Buf(pval* out_buffer, const byte* src)
{
 ...
 static unsigned char *pv=0;
 static unsigned char pvbuf[256];
 int count = SCREENHEIGHT * SCREENWIDTH;
 if(pv==0){ // need to translate to shorts
    int cnt=0;
	pv=pvbuf;
    while(cnt<256){
		pvbuf[cnt]=cnt;
 ...
 ICVM_BltTranslateScaleCM8To8(optr,iptr,pv,
...
static void I_Copyto16Buf(pval* out_buffer, byte* src)
{
  register  byte* iptr =  src;
  register unsigned short* optr = (unsigned short*)out_buffer;
#if defined(INTERNETC)
 static unsigned short *pv=0;
 static unsigned short pvbuf[256];
 int count = SCREENHEIGHT * SCREENWIDTH;
 if(pv_changed || pv==0){ // need to translate to shorts
    int cnt=0;
	pv=pvbuf;
    while(cnt<256){
		pvbuf[cnt]=pixelvals[cnt];
        ++cnt;
    }
 }
 ICVM_BltTranslateCM8To16(optr,iptr,count,pv);
#else
...
static void I_Double16Buf(pval* out_buffer, const byte* src)
{
 ...
 ICVM_BltTranslateScaleCM8To16(optr,iptr,pv,
	SCREENWIDTH,SCREENHEIGHT,
	SCREENWIDTH*2,SCREENHEIGHT*2,
	0x8000,0,
	0,0x8000,
...
#if defined(I386) || defined(INTERNETC)
...
static void I_Copyto32Buf(pval* out_buffer, byte* src)
{
  register byte* iptr = src;
...
#if defined(INTERNETC)
  int count = SCREENHEIGHT * SCREENWIDTH;
	ICVM_BltTranslateCM8To32(optr,iptr,count,pixelvals);
#else
  int count = SCREENHEIGHT * SCREENWIDTH / 4;
...
static void I_Double32Buf(pval* out_buffer, const byte* src)
{
 ...
 ICVM_BltTranslateScaleCM8To32(optr,iptr,pv,
	SCREENWIDTH,SCREENHEIGHT,
	SCREENWIDTH*2,SCREENHEIGHT*2,
	0x8000,0,
	0,0x8000,
 ...
 *
 * Revision 1.3  2000/09/28 18:28:45  bob
 * Port distribution
 *
 * Revision 1.2  2000/09/23 05:05:23  bob
 * Faster
 *
 * Revision 1.1  2000/09/22 13:19:37  bob
 * Really Bad Fucking Program -- this CVS
 *
 * Revision 1.4  2000/09/19 14:19:35  bob
 * More optimization
 *
 * Revision 1.3  2000/08/19 15:46:25  bob
 * Fresh update to old source
 *
 * Revision 1.1  2000/08/07 18:16:18  bob
 * Doom port
 *
