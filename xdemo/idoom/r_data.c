 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: r_data.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $
 *
...
static const char
rcsid[] = "$Id: r_data.c,v 1.1.1.1 2001/08/23 10:48:18 bob Exp $";
...
#ifdef INTERNETC
  source = alloca(texture->height);       // temporary column - local allocation
#else
...
#ifdef INTERNETC // allocate locally
  struct {
    unsigned short patches, posts;
  } *count = alloca(sizeof *count * texture->width);
	memset(count,0,sizeof *count * texture->width);
#else
  struct {
    unsigned short patches, posts;
...
#ifdef INTERNETC // use local allocation
...
// put back alloca usage. Why do you care where the mem comes from?
// alloca has much less overhead and won't fail. Especially if you
// are running on an 8 meg stack with an app like Doom.  INTERNETC
...
#ifdef INTERNETC // use local allocation
    hitlist = alloca(numtextures > size ? numtextures : size);
#else
...
//
// Revision 1.3  2001/02/24 01:05:36  bob
// Bug fixes
//
