...
#ifdef INTERNETC
            if (key==XK_KP_Up) newdir=UP;
   	        if (key==XK_KP_Right) newdir=LEFT; // seem to be backwards
       	    if (key==XK_KP_Down) newdir=DOWN;
           	if (key==XK_KP_Left) newdir=RIGHT; // seem to be backwards
            if (key==XK_Up) newdir=UP;
   	        if (key==XK_Right) newdir=LEFT; // seem to be backwards
       	    if (key==XK_Down) newdir=DOWN;
           	if (key==XK_Left) newdir=RIGHT; // seem to be backwards
#endif
      ...
      if (*(dmap+((x/16)*height)+(y/16)) == DIRTY ) {
        ...
        *(dmap+((x/16)*height)+(y/16)) = CLEAN;
        ...
        switch (*(map+((x/16)*height)+(y/16))) {
          ...
          case EMPTY : 
			XPutImage(display,win,gc,empty,0,0,x,y,16,16); 
			break;
          case WALL : 
			XPutImage(display,win,gc,wall,0,0,x,y,16,16); 
			break;
          case PILL : 
			XPutImage(display,win,gc,pill,0,0,x,y,16,16); 
			break;
          case EATPILL : 
			XPutImage(display,win,gc,eatpill,0,0,x,y,16,16);
			break; 
