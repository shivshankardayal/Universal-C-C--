     ...
     rtx x;
{
  return ((GET_CODE (x) == SUBREG
	   ...
	   && subreg_lowpart_p (x)
#ifdef GCC_ICVM
		&& IsICVMNopTrunc(SUBREG_REG(x),GET_MODE(x))
...
fprintf(stderr,"@@@ cse.c lowpart10\n");
...
fprintf(stderr,"@@@ cse.c lowpart20\n");
...
#ifdef GCC_ICVM
		&& GET_MODE(qty_const[REG_QTY (REGNO (arg))]) == GET_MODE(arg)
#endif
		)
	      const_arg
...
fprintf(stderr,"@@@ cse.c lowpart200\n");
...
fprintf(stderr,"@@@ cse.c lowpart300\n");
  ...
  if (code == NE && GET_CODE (op0) == SUBREG
	  ...
	  < GET_MODE_SIZE (GET_MODE (SUBREG_REG (op0)))))
    {
      enum machine_mode inner_mode = GET_MODE (SUBREG_REG (op0));
      rtx tem = gen_lowpart_if_possible (inner_mode, op1);
fprintf(stderr,"@@@ cse.c lowpart30\n");
  ...
  if (code == NE && GET_CODE (op1) == SUBREG
	  ...
	  < GET_MODE_SIZE (GET_MODE (SUBREG_REG (op1)))))
    {
      enum machine_mode inner_mode = GET_MODE (SUBREG_REG (op1));
      rtx tem = gen_lowpart_if_possible (inner_mode, op0);
fprintf(stderr,"@@@ cse.c lowpart400\n");
...
0 &&
#endif
		src_const && src_related == 0 && GET_CODE (src_const) == CONST_INT
	  && GET_MODE_CLASS (mode) == MODE_INT
	  && GET_MODE_BITSIZE (mode) < BITS_PER_WORD)
	{
	  ...
	  for (wider_mode = GET_MODE_WIDER_MODE (mode);
		...
		if (GET_CODE (const_elt->exp) == REG)
		  {
fprintf(stderr,"@@@ cse.c lowpart500\n");
...
0 &&
		...
		flag_expensive_optimizations && ! src_related
...
fprintf(stderr,"@@@ cse.c lowpart550\n");
	      ...
	      if (inner)
		{
		  ...
		  for (larger_elt = larger_elt->first_same_value;
		    ...
		    if (GET_CODE (larger_elt->exp) == REG)
		      {
fprintf(stderr,"@@@ cse.c lowpart600\n");
			src_related
	  ...
	  && LOAD_EXTEND_OP (mode) != NIL)
	{
	  ...
	  for (tmode = GET_MODE_WIDER_MODE (mode);
		...
		if (GET_CODE (larger_elt->exp) == REG)
		  {
fprintf(stderr,"@@@ cse.c lowpart700\n");
		    src_related = gen_lowpart_if_possible (mode, 
							   larger_elt->exp);
		    break;
		  }
...
fprintf(stderr,"@@@ cse.c lowpart900\n");
	  if (new)
	    return new;
	}
#endif
