...
#ifdef GCC_ICVM
  add_attribute (A_SYSCALL, "syscall", 3, 3, 1);
#endif
     ...
     tree node, attributes, prefix_attributes;
{
  ...
  for (a = attributes; a; a = TREE_CHAIN (a))
	...
	case A_SYSCALL:
	  {
	    tree operand = TREE_VALUE (args);
	    tree preamble = TREE_VALUE (TREE_CHAIN(args));
	    tree postop= TREE_VALUE (TREE_CHAIN (TREE_CHAIN (args)));
		char *funcname=IDENTIFIER_POINTER(DECL_NAME(decl));
		char *funcassemblername=
	    ...
	    if (TREE_CODE (decl) != FUNCTION_DECL)
	      {
		error_with_decl (decl,
			 "syscall specified for non-function `%s'");
		continue;
	      }

	    if (TREE_CODE (operand) != INTEGER_CST){
			error ("syscall operand is non-constant");
			continue;
		}

	    operandnum = TREE_INT_CST_LOW (operand);

	    if (TREE_CODE (preamble) != STRING_CST
			|| TREE_CODE (postop) != STRING_CST){


if(TREE_CODE(preamble)==STRING_CST)
fprintf(stderr,"preamblenum is a string - [%s]\n",TREE_STRING_POINTER(preamble));

if(TREE_CODE(postop)==STRING_CST)
fprintf(stderr,"firstarg is a string - [%s]\n",TREE_STRING_POINTER(postop));

if(TREE_CODE(preamble)==ARRAY_TYPE)
fprintf(stderr,"preamblenum is an array\n");


fprintf(stderr,"TREECODE=%d %d\n",TREE_CODE(preamble),TREE_CODE(postop));

			error ("syscall's preamble/postop must be strings");
			continue;
	      }

	    preamblestr = TREE_STRING_POINTER(preamble);
	    postopstr = TREE_STRING_POINTER(postop);
		icvm_addsyscall(funcname,operandnum,preamblestr,postopstr);
	    break;
	  }
...
#define T_LLL	&long_long_long_integer_type_node
#define T_S	&short_integer_type_node
#define T_UI	&unsigned_type_node
#define T_UL	&long_unsigned_type_node
#define T_ULL	&long_long_unsigned_type_node
#define T_ULLL	&long_long_long_unsigned_type_node
