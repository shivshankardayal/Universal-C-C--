...
void convert_move (to, from, unsignedp)
     ...
     int unsignedp;
{
...
#ifdef GCC_ICVM
	if(GET_MODE_SIZE(from_mode)==4 && GET_MODE_SIZE(to_mode)==4){
		if(from_mode==SFmode || to_mode==SImode){
	  		emit_unop_insn (CODE_FOR_movsfsi, to, from, UNKNOWN);
	  		return;
		}else if(from_mode==SImode || to_mode==SFmode){
	  		emit_unop_insn (CODE_FOR_movsisf, to, from, UNKNOWN);
			return;
		}else
			abort();
	}else if(GET_MODE_SIZE(from_mode)==8 && GET_MODE_SIZE(to_mode)==8){
		if(from_mode==DFmode || to_mode==DImode){
	  		emit_unop_insn (CODE_FOR_movdfdi, to, from, UNKNOWN);
	  		return;
		}else if(from_mode==DImode || to_mode==DFmode){
	  		emit_unop_insn (CODE_FOR_movdidf, to, from, UNKNOWN);
			return;
		}else
			abort();
	}else
  ...
  if (GET_CODE (to) == SUBREG && SUBREG_PROMOTED_VAR_P (to)){
	fprintf(stderr,"convert_move in expr.c problem\n");
    abort ();
  }	
...
{
  rtx r;
  r=convert_modes (mode, VOIDmode, x, unsignedp);
  return r;
}
...
#ifdef GCC_ICVM
      && move_by_pieces_ninsns (len, align) > (MOVE_MAX_PIECES/MOVE_MAX))
#else
...
#ifndef GCC_ICVM_NOTNOW /* ICVM - MIGHT ??? causes alignment problems */
...
#ifdef GCC_ICVM
      && (mode = mode_for_size (size * BITS_PER_UNIT, MODE_INT, 1)) != BLKmode)
#else
...
#ifndef GCC_ICVM_NOTNOW /* ICVM - MIGHT ??? cause alignment problems */
  if (! SLOW_UNALIGNED_ACCESS
      || align > MOVE_MAX || align >= BIGGEST_ALIGNMENT / BITS_PER_UNIT)
    align = MOVE_MAX;
#endif
  ...
  /* set constructor assignments */
	...
	abort ();
	 ...
	 constant parts) by copying from a constant.  */	 
      if (
	GET_MODE (target) != BLKmode || nbits <= 2 * BITS_PER_WORD
...
/*    case BUILT_IN_FABS: */
	...
	case BUILT_IN_FABS:
	  builtin_optab = fabs_optab; break;
	default:
	  abort ();
	}
...
#ifdef GCC_ICVM
	  && (mode = mode_for_size (i + 1, MODE_INT, 1)) != BLKmode
#else
