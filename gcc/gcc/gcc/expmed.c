...
#ifdef GCC_ICVM /* ICVM bigger than byte handling for bitfields */
  /* Handle bitfields bigger than a byte.  */
  
  if (isbitfield && bitsize > BITS_PER_UNIT)
    {
	  int bits=bitsize;
	  int bit_offset=bitnum;
	  int bsz=0;
	  int shift=0;
      ...
      while(bits)
	{
		  rtx shifted=value;
		  int bp=bit_offset % 8;
		  bsz=MIN (BITS_PER_UNIT,bits);
		  bsz=MIN (bsz,BITS_PER_UNIT-(bit_offset % 8));
		

	  if(shift){
			if(GET_CODE(shifted)==CONST_INT)
				shifted=GEN_INT(INTVAL(shifted)>>shift );
			else{
				shifted=force_simpleoperand(shifted);
				shifted=gen_rtx_ASHIFTRT(GET_MODE(shifted),
					shifted,GEN_INT(shift));
			}
	  }
	  if(GET_CODE(shifted)==CONST_INT)
			shifted=GEN_INT(INTVAL(shifted) & ((1<<bsz)-1));
#ifdef NOT_NOW
//	  if(GET_MODE(shifted)!=fieldmode)
//	   		shifted=ConvertToMode(fieldmode,shifted,0);
//	     	shifted= expand_shift (RSHIFT_EXPR,
//					GET_MODE(shifted),shifted,
//			     build_int_2 (shift, 0),
//			     NULL_RTX, 0);
#endif
	   store_bit_field(str_rtx,bsz,bit_offset,fieldmode,shifted,align,total_size,isbitfield);
...
fprintf(stderr,"@@@ EXMPMED genlowpart1\n");
...
fprintf(stderr,"@@@ EXMPMED genlowpart2\n");
...
fprintf(stderr,"@@@ EXPMED LOWPART 3\n");
  ...
  if (isbitfield && bitsize > BITS_PER_UNIT)
      ...
      while(bits)
	{
	  ...
	  bsz=MIN (bsz,BITS_PER_UNIT-(bit_offset % 8));
	   ...
	   result_part = extract_bit_field (str_rtx, bsz,
			bit_offset,
			bits!=bsz ? 1 : unsignedp, target_part, GET_MODE(target),GET_MODE(target),
			align, total_size,1
	  );


	  if(shift){
      		result_part= expand_shift (LSHIFT_EXPR,
					GET_MODE(result_part),result_part,
			     build_int_2 (shift, 0),
			     NULL_RTX, 0);
			target=expand_binop(GET_MODE(target),ior_optab,
				target,result_part,
					target,1,OPTAB_LIB_WIDEN);			
	 }else{
	  	if (result_part != target)
	    	emit_move_insn (target, result_part);
	 }
	 shift+=bsz;
	 bit_offset+=bsz;
	 bits-=bsz;
	 target_part=0;
      ...
      if (unsignedp)
	{
      /* unsigned bit field: zero-extend with two arithmetic shifts.  */
...
fprintf(stderr,"@@@ EXMPMED gen_lowpart6\n");
	  op0 = gen_lowpart (imode, op0);
	}else
	  abort ();
      }
	...
	&& TRULY_NOOP_TRUNCATION (GET_MODE_BITSIZE (mode),
	  ...
	  || (mode_for_size (bitsize, GET_MODE_CLASS (tmode), 0) != BLKmode
		 ...
		 inefficient code from the Thumb port when -mbig-endian.  */
	      && (BYTES_BIG_ENDIAN
#ifdef GCC_ICVM
&& 0 /* bad BigEndian test - see notes above */
#endif
  ...
  if (bitsize > BITS_PER_WORD)
      ...
      for (i = 0; i<nwords; i++)
	{
	  ...
	  rtx target_part=0;
	  rtx result_part;
	  target_part = operand_subword (target, wordnum, 1, VOIDmode);
...
fprintf(stderr,"@@@ EXMPMED genlowpart7\n");
		...
		&& (bitsize + bitpos > extv_bitsize)))
	{
	  ...
	  if (GET_MODE (xtarget) != maxmode)
	      ...
	      if (GET_CODE (xtarget) == REG)
		{
		  int wider = (GET_MODE_SIZE (maxmode)
			       > GET_MODE_SIZE (GET_MODE (xtarget)));
fprintf(stderr,"@@@ EXMPMED genlowpart8\n");
		  xtarget = gen_lowpart (maxmode, xtarget);
		  if (wider)
		    xspec_target_subreg = xtarget;
		}
  ...
  if (op1 != const0_rtx)
		...
		if (EXACT_POWER_OF_2_OR_ZERO_P (d))
		  {
			...
			if (remainder){
#ifdef GCC_ICVM
	    	return ConvertToMode(mode,remainder, unsignedp);
#else
			  	return gen_lowpart (mode, remainder); /* not in ICVM */
#endif
		    }	 
		   }
	...
	if (expand_twoval_binop (sdivmod_optab, op0, op1,
				 quotient, remainder, 0))
	  {
	    ...
	    emit_label (label);
#ifdef GCC_ICVM
	    	return ConvertToMode(mode,rem_flag ? remainder:quotient, unsignedp);
#else
	    return gen_lowpart (mode, rem_flag ? remainder : quotient); /* not in ICVM */
#endif
	  }
