...
static boolean check_for_scripts_dir PARAMS ((char *dir, boolean append));
...
set_scripts_dir ()
{
  ...
  if (command_line.multilib_dir != NULL)
    {
      if (*command_line.multilib_dir == '/')
	{
	  dir = xmalloc (strlen (command_line.multilib_dir) + 1);
	  strcpy (dir, command_line.multilib_dir);
	}
      else
	{
	  dir = xmalloc (strlen (SCRIPTDIR)
			 + strlen (command_line.multilib_dir) + 2);
	  strcpy (dir, SCRIPTDIR);
	  strcat (dir, "/");
	  strcat (dir, command_line.multilib_dir);
	}

      if (check_for_scripts_dir (dir, false))
	return;
      else
	free (dir);
    }
