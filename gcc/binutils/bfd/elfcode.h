     ...
     Elf_Internal_Ehdr *dst;
{
  ...
  memcpy (dst->e_signature, src->e_signature, sizeof(src->e_signature));
     ...
     Elf_Internal_Shdr *dst;
{
	...
	strcpy(dst->sh_sectionname,src->sh_sectionname);

/*  dst->sh_name = bfd_h_get_32 (abfd, (bfd_byte *) src->sh_name); ICVM */
     ...
     Elf_External_Shdr *dst;
{
  /* note that all elements of dst are *arrays of unsigned char* already... */
	strcpy(dst->sh_sectionname,src->sh_sectionname);
/*  bfd_h_put_32 (abfd, src->sh_name, dst->sh_name); ICVM */
     ...
     Elf_External_Ehdr *x_ehdrp;
{
  return ((x_ehdrp->e_ident[EI_MAG0] == ELFMAG0)
	  ...
	  && (x_ehdrp->e_ident[EI_MAG4] == ELFMAG4)
	  && (x_ehdrp->e_ident[EI_MAG5] == ELFMAG5)
	  && (x_ehdrp->e_ident[EI_MAG6] == ELFMAG6));
}
     ...
     PTR data;
{
  ...
  rela_hdr->contents = (PTR) bfd_zalloc (abfd, rela_hdr->sh_size);
     ...
     boolean dynamic;
{
  ...
  if (symcount == 0)
	...
	goto error_return;
      ...
      if (verhdr != NULL
	  && verhdr->sh_size / Elf_External_Versym_sizeof != symcount)
	{
	  (*_bfd_error_handler)
	    ("%s: version count (%ld) does not match symbol count (%ld)",
	     abfd->filename,
	     (long) (verhdr->sh_size / Elf_External_Versym_sizeof),
	     symcount);
	  ...
	  if (x_versymp != NULL)
	    {
	      ...
	      _bfd_elf_swap_versym_in (abfd, (Elf_External_Versym *)
		 ((char *) x_versymp + i * Elf_External_Versym_sizeof),
				       &iversym);
