     ...
     const char *name;
{
  ...
  if ((hdr->sh_flags & SHF_EXECINSTR) != 0)
    flags |= SEC_CODE;
  if ((hdr->sh_flags & SHF_BIGENDIAN) != 0)
    flags |= SEC_BIGENDIAN;
     ...
     char *name;
{
  ...
  if (i_shdrp != NULL)
      ...
      if (shstrtab != NULL)
	{
	  ...
	  for (i = 1; i < max; i++)
	    if (!strcmp (i_shdrp[i]->sh_sectionname, name))
	      return i_shdrp[i];
	}
    }
  return 0;
     ...
     PTR failedptrarg;
{
...
#endif
  strcpy(this_hdr->sh_sectionname,asect->name);
  ...
  if (strcmp (asect->name, ".dynstr") == 0)
  ...
  else if (strcmp (asect->name, ".gnu.version") == 0)
    {
      ...
      this_hdr->sh_entsize = Elf_External_Versym_sizeof;
    }
  ...
  if ((asect->flags & SEC_BIGENDIAN) != 0)
    this_hdr->sh_flags |= SHF_BIGENDIAN;
...
#endif
     strcpy(rela_hdr->sh_sectionname,name);
     ...
     bfd *abfd;
{
  ...
  memset(i_ehdrp->e_ident,0x7f,EI_NIDENT);
  memset(i_ehdrp->e_signature,0,sizeof(i_ehdrp->e_signature));

  strcpy(i_ehdrp->e_ident,ICVMELFSHHEADER);
  i_ehdrp->e_ident[EI_MAG0] = ELFMAG0;
  i_ehdrp->e_ident[EI_MAG1] = ELFMAG1;
  i_ehdrp->e_ident[EI_MAG2] = ELFMAG2;
  i_ehdrp->e_ident[EI_MAG3] = ELFMAG3;
  i_ehdrp->e_ident[EI_MAG4] = ELFMAG4;
  i_ehdrp->e_ident[EI_MAG5] = ELFMAG5;
  i_ehdrp->e_ident[EI_MAG6] = ELFMAG6;

  i_ehdrp->e_ident[EI_CLASS] = bed->s->elfclass;
  i_ehdrp->e_ident[EI_DATA] =
    bfd_big_endian (abfd) ? ELFDATA2MSB : ELFDATA2LSB;
  i_ehdrp->e_ident[EI_VERSION] = bed->s->ev_current;
  t=time(0); /* time stamp the file */
  i_ehdrp->e_ident[EI_TIME1] = t>>24;
  i_ehdrp->e_ident[EI_TIME2] = t>>16;
  i_ehdrp->e_ident[EI_TIME3] = t>>8;
  i_ehdrp->e_ident[EI_TIME4] = t;
  ...
  if (elf_tdata (abfd)->symtab_hdr.sh_name == (unsigned int) -1
    ...
    return false;
#endif
  strcpy(elf_tdata (abfd)->symtab_hdr.sh_sectionname,".symtab");
  strcpy(elf_tdata (abfd)->strtab_hdr.sh_sectionname,".strtab");
  strcpy(elf_tdata (abfd)->shstrtab_hdr.sh_sectionname,".shstrtab");
...
#define IS_SOLARIS_PT_INTERP(p, s)					\
  ...
  for (i = 0, p = elf_tdata (ibfd)->phdr; i < num_segments; i++, p++)
      ...
      if (csecs == 0)
	{
	  ...
	  if (p->p_type == PT_LOAD)
	      _bfd_error_handler
		("%s: warning: Empty loadable segment detected\n",
