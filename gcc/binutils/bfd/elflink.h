     ...
     struct bfd_link_info *info;
{
  ...
  weaks = NULL;
  ever = extversym != NULL ? (Elf_External_Versym *)
    ((char *) extversym + extsymoff * Elf_External_Versym_sizeof) : NULL;
  esymend = buf + extsymcount;
  for (esym = buf;
       esym < esymend;
       esym++, sym_hash++,
	 ever = (ever != NULL ? (Elf_External_Versym *)
		 ((char *) ever + Elf_External_Versym_sizeof) : NULL))
    {
      Elf_Internal_Sym sym;
      int bind;
      bfd_vma value;
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	if (elf_tdata (output_bfd)->verref == NULL)
	...
	else
	  {
	    ...
	    s->_raw_size = size;
	    s->contents = (bfd_byte *) bfd_zalloc (output_bfd, size);
	    if (s->contents == NULL)
	      return false;
	...
	{
	  s->_raw_size = dynsymcount * Elf_External_Versym_sizeof;
	  s->contents = (bfd_byte *) bfd_zalloc (output_bfd, s->_raw_size);
	  if (s->contents == NULL)
	    return false;
  ...
  if (h->dynindx != -1
      ...
      if (finfo->symver_sec != NULL && finfo->symver_sec->contents != NULL)
	{
	  ...
	  _bfd_elf_swap_versym_out (finfo->output_bfd, &iversym,
	    (Elf_External_Versym *) ((char *) finfo->symver_sec->contents
	    + h->dynindx * Elf_External_Versym_sizeof));
	}
    }
