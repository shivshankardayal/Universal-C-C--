     ...
     arelent **relppend;
{
  ...
  info->current_endian=BFD_ENDIAN_LITTLE;
  if(section->flags & SEC_BIGENDIAN)	  
	  info->current_endian=BFD_ENDIAN_BIG;
	  ...
	  if (insns)
		...
		bytes_per_line = info->bytes_per_line;
	      if (bytes < 0)
	        {
		  if (buf [0] != '\0')
		    /* We check if `buf` is empty. We may have some
