...
struct section_list
{
  ...
  boolean remove;
  /* Wheter to extract this section (if --section
     is specified, all other sections are removed) */
  boolean extract;
...
static boolean sections_removed, sections_extracted;
...
static struct option copy_options[] =
{
  ...
  {"discard-all", no_argument, 0, 'x'},
  {"discard-locals", no_argument, 0, 'X'},
  {"extract-section", required_argument, 0, 'j'},
  {"format", required_argument, 0, 'F'}, /* Obsolete */
     ...
     asection *sec;
{
  ...
  if (! sections_removed && ! sections_extracted)
    return false;
  p = find_section_list (bfd_get_section_name (abfd, sec), false);
  return ((p && p->remove && sections_removed) ||
     ...
     PTR obfdarg;
{
  ...
  if ((p && p->remove && sections_removed) ||
      (!(p && p->extract) && sections_extracted))
    return;
