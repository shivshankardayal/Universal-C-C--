...
static CONST struct asm_flg lfm_flags[] =
{
  {"fd", 0x00800000},
  {"ea", 0x01000000},
  /* Should we support "ia", "db" etc as synonyms?  Lack of these has
     caught me out at least once.  --pb */
  {NULL, 0}
};
...
static void arm_elf_cons PARAMS ((int));
...
#ifdef OBJ_ELF
	    /* Setting this to 0 should work, but it seems to get added 
	       in again later so we need to cancel that out.  
	       I suspect the ELF backend is the culprit.  */
...
/* Added case statement for PLT32 case. */
#ifdef OBJ_ELF
...
#ifdef OBJ_ELF
  /* RMK: for ELF.  Convert a .long _GLOBAL_OFFSET_TABLE_-[.Lx+8] to
   * an offsetable reloc.
   */
 if (BFD_RELOC_32_PCREL == code 
     && GOT_symbol
