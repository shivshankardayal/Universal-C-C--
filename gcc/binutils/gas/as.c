...
print_version_id ()
{
  ...
  fprintf (stderr, "GNU assembler version %s (%s)", ICVM_VERSION, TARGET_ALIAS);
#ifdef BFD_ASSEMBLER
     ...
     char **argv;
{
  ...
  Atext_section = subseg_new (TEXT_SECTION_NAME, 0);
  Adata_section = subseg_new (DATA_SECTION_NAME, 0);
  Abss_section = subseg_new (BSS_SECTION_NAME, 0);


  LEtext_section = subseg_new (LTEXT_SECTION_NAME, 0);
  LEdata_section = subseg_new (LDATA_SECTION_NAME, 0);
  LErdata_section = subseg_new (LRDATA_SECTION_NAME, 0);
  LEctors_section = subseg_new (LCTORS_SECTION_NAME, 0);
  LEdtors_section = subseg_new (LDTORS_SECTION_NAME, 0);
  LEbss_section = subseg_new (LBSS_SECTION_NAME, 0);
  BEtext_section = subseg_new (BTEXT_SECTION_NAME, 0);
  BEdata_section = subseg_new (BDATA_SECTION_NAME, 0);
  BErdata_section = subseg_new (BRDATA_SECTION_NAME, 0);
  BEctors_section = subseg_new (BCTORS_SECTION_NAME, 0);
  BEdtors_section = subseg_new (BDTORS_SECTION_NAME, 0);
  BEbss_section = subseg_new (BBSS_SECTION_NAME, 0);

  bfd_set_section_alignment (stdoutput,Atext_section,2);
  bfd_set_section_alignment (stdoutput,Adata_section,2);
  bfd_set_section_alignment (stdoutput,Abss_section,2);

  bfd_set_section_alignment (stdoutput,LEtext_section,2);
  bfd_set_section_alignment (stdoutput,LEdata_section,2);
  bfd_set_section_alignment (stdoutput,LErdata_section,2);
  bfd_set_section_alignment (stdoutput,LEctors_section,2);
  bfd_set_section_alignment (stdoutput,LEdtors_section,2);
  bfd_set_section_alignment (stdoutput,LEbss_section,2);

  bfd_set_section_alignment (stdoutput,BEtext_section,2);
  bfd_set_section_alignment (stdoutput,BEdata_section,2);
  bfd_set_section_alignment (stdoutput,BErdata_section,2);
  bfd_set_section_alignment (stdoutput,BEctors_section,2);
  bfd_set_section_alignment (stdoutput,BEdtors_section,2);
  bfd_set_section_alignment (stdoutput,BEbss_section,2);
...
#ifdef TC_ICVM
  bfd_set_section_flags (stdoutput, BEtext_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC
				       | SEC_CODE | SEC_READONLY | SEC_BIGENDIAN));
  bfd_set_section_flags (stdoutput, LEtext_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC
				       | SEC_CODE | SEC_READONLY));
  bfd_set_section_flags (stdoutput, Atext_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC
				       | SEC_CODE | SEC_READONLY));
  /* @@ FIXME -- SEC_CODE seems to mean code only, rather than code possibly.*/
  bfd_set_section_flags (stdoutput, BEdata_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_BIGENDIAN));
  bfd_set_section_flags (stdoutput, LEdata_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC));
  bfd_set_section_flags (stdoutput, Adata_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC));
  bfd_set_section_flags (stdoutput, BErdata_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY | SEC_BIGENDIAN));
  bfd_set_section_flags (stdoutput, LErdata_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY));
  bfd_set_section_flags (stdoutput, BEdtors_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY | SEC_BIGENDIAN));
  bfd_set_section_flags (stdoutput, LEdtors_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY));
  bfd_set_section_flags (stdoutput, BEctors_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY | SEC_BIGENDIAN));
  bfd_set_section_flags (stdoutput, LEctors_section,
			 applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY));
  bfd_set_section_flags (stdoutput, BEbss_section, applicable & SEC_ALLOC);
  bfd_set_section_flags (stdoutput, LEbss_section, applicable & SEC_ALLOC);
  bfd_set_section_flags (stdoutput, Abss_section, applicable & SEC_ALLOC);
  seg_info (BEbss_section)->bss = 1;
  seg_info (LEbss_section)->bss = 1;
  seg_info (Abss_section)->bss = 1;
...
#endif
#ifdef TC_ICVM
#ifdef ICVM_ENABLE_DUAL_ENDIAN

{
 int oldargc=argc;
 char **oldargv=argv;
 target_big_endian=0;
while(target_big_endian<2){
  	argc=oldargc;
    argv=oldargv;
